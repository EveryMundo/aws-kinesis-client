...
const sendItToKinesisWithRetry = async (params, kinesisClient = kinesis.client, maxRetries = KINESIS_MAX_RETRIES, attempt = 1) => {
  if (attempt > maxRetries) {
    throw new Error(`sendItToKinesis ${params.StreamName} failed after ${attempt} attempts`)
  }

  const kinesisResponse = await kinesisClient.putRecords(params).promise()
  ...
  if (kinesisResponse.FailedRecordCount === params.Records.length) {
    ...
    return delayedCall(attempt * KINESIS_RETRY_DELAY, () => sendItToKinesisWithRetry(params, kinesisClient, maxRetries, attempt + 1))
  }
  ...
  return delayedCall(attempt * KINESIS_RETRY_DELAY, () => sendItToKinesisWithRetry({ StreamName, Records }, kinesisClient, maxRetries, attempt + 1))
}

const sendItCompressedToKinesisWithRetry = async (params, kinesisClient, maxRetries, thresholdInBytes = 25600) => {
  ...
  return sendItToKinesisWithRetry(params, maxRetries, kinesisClient)
...
const sendItOptimizedToKinesisWithRetry = async (StreamName, Docs, kinesisClient, maxRetries) => {
  const params = {
    StreamName,
  ...
  // return params
  return sendItToKinesisWithRetry(params, kinesisClient, maxRetries)
}
